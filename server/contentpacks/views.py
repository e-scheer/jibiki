"""Serve generated SQLite packs from the runtime artifact directory."""
...
def _packs_manifest() -> dict | None:
    path = _pack_dir() / "packs_manifest.json"
    if not path.exists():
        return None
...
def packs_file(request, name: str):
    try:
        ...
        raise Http404("invalid pack manifest") from None
    # Only serve files declared by the validated manifest.
